  ...
  for (; state.index < chunkLength; state.index++) {
    ...
    if (!state.comment.parsing && !state.quote.open &&
      ...
      if (tempValue || (currentKey && !skipEmptyValues)) {
        ...
        if (state.evaluation.parsingDependencies) {
          ...
          addValueToStructure(ext_variables, currentKey, trimWrappingQuotes(tempValue), true);
        }
      }
    ...
    if (character === CHAR_NEWLINE) {
      ...
      if (tempString || (currentKey && !skipEmptyValues)) {
        ...
        if (state.evaluation.parsingDependencies) {
          ...
          addValueToStructure(ext_variables, currentKey, trimWrappingQuotes(tempString), true);
        }
    ...
    if (character === CHAR_BLOCK_START) {
      ...
      if (EXT_KEYS.indexOf(currentKey) > -1) {
        ...
        for (var property in out[currentKey])
          if (out[currentKey].hasOwnProperty(property)) {
            ...
            addValueToStructure(ext_variables, property, propertyValue, true);
          }
      } else if (!currentKey) {
        for (var property in out[currentKey]) {
          if (out[currentKey].hasOwnProperty(property) && property.startsWith('ext.')) {
            var propertyValue = out[currentKey][property];
            addValueToStructure(ext_variables, property.slice(4), propertyValue, true);
          }
        }
      }
      currentKey = '';
      ...
      if (isKeyword(tempString)) {
        if (tempString === KEYWORD_DEF || tempString === KEYWORD_STRING || tempString === KEYWORD_FINAL) {
...
function isKeyword(string) {
  return string === KEYWORD_DEF || string === KEYWORD_IF || string === KEYWORD_STRING || string === KEYWORD_FINAL;
}
...
function addValueToStructure(structure, currentKey, value, replace) {
